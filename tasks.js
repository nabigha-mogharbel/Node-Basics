...
add title: add todo to your todo list
edit x title: edit title of todo list number x
remove x: remove todo number x
check x: mark todo number x as done
uncheck x: mark todo number x as undone`
  );
}
 ...
 */
function list() {
  parsedData.todoList.map((todo, index) => {
    todo.done === false
      ? console.log(`[${(index = index + 1)}] [ ] ${todo.todo}`)
      : console.log(`[${(index = index + 1)}] [✓] ${todo.todo}`);
  });
}
...
function add(todo) {
  todo.length != 0
    ? parsedData.todoList.push({ todo: todo, done: false })
    : console.log("error!: please enter a todo");
}
...
function remove(index) {
  ...
  if (!index && stringIndx != "0") {
    console.log(index);
    parsedData.todoList.pop();
  } else if (index <= 0 || index > parsedData.todoList.length) {
...
/**
 * edit => error!
 * edit x title => edit title of your todo number x in todo list
 * edit title => edit title of your  last todo in todo list
 * @param  {string} index the text received
 * @returns {void}
 */
function edit(newTitle) {
  ...
  if (index && parsedData.todoList.length >= index) {
    ...
    parsedData.todoList[parsedData.todoList.length - 1].todo =
      trimmedTitle.join(" ");
  } else {
    console.log("error!: please enter a number that exists in your todo list");
  }
...
function check(index) {
  ...
  index = parseInt(index);
  if (!index && stringIndx != "0") {
    console.log("error!: please enter a number that exist your todo list");
  } else if (index <= 0 || index > parsedData.todoList.length) {
    console.log("error!: please enter a number that exists in your todo list");
  } else {
